function [hfig,savename,sortederrs] = PlotFracInliersPerLandmark_ARannotator(varargin)
...
%   
% end
sortederrs(1,:,:) = GetSortedErrs(cur_data,1:nlandmarks,net,exptype,conddata,datatypes,labeltypes,statname);
...
if ~isempty(annoterrdata),
  ...
  for i = 1:numel(annfns),
    ...
    conddata.data_cond = ones(numel(annoterrdata.(annfns{i}){end}.data_cond),1);
    conddata.label_cond = ones(numel(annoterrdata.(annfns{i}){end}.label_cond),1);
    annotsortederrs(i,:,:) = GetSortedErrs(cur_data,1:nlandmarks,'',exptype,conddata,datatypes,labeltypes,statname);
  end
...
for datai = ndatatypes:-1:1,
  ...
  for labeli = 1:nlabeltypes,
    ...
    if labeli == nlabeltypes && datai == ndatatypes,
      legend(h,alllegendnames,'Location','East');
    end
...
if dosavefig,
  if isempty(savename),
    savename = fullfile(savedir,sprintf('%s_GTFracInliers_%s_AnnotatatorsvsGT',exptype,statname));
  end
