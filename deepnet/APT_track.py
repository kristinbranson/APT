...
## Find all the stored models
    ...
    # Use tracking_config.json from the lbl bundle if present and no explicit -config_file given.
    # This file is generated by Matlab's projBundleSave_ using trkCreateConfig (fortracking=True),
    # capturing the tracking params at save time without addExtraParams.
    if args.config_file is None:
        bundled_trk_config = os.path.join(tdir, 'tracking_config.json')
        if os.path.exists(bundled_trk_config):
            argv = argv + ['-config_file', bundled_trk_config]
