...
class Pose_mmpose(PoseCommon_pytorch.PoseCommon_pytorch):
    ...
    def train_wrapper_old(self, restore=False, model_file=None, debug=False, logger=None):
        ...
        if model_file is not None:
            load_from = model_file
            resume = False
        elif restore:
            resume = True
            load_from = self.get_latest_model_file()
        ...
        if mmpose_new:
            dataloaders = CombinedDataset(ds, **dataloader_setting)
        else:
