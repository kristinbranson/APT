...
from tensorflow.contrib.layers.python.layers import regularizers, initializers
from tensorflow.contrib.layers.python.layers import layers as tflayers
...
class HourglassModel():
    ...
    def __init__(self, nFeat=512, nStack=1, nModules=1, nLow=4, outputDim=16, batch_size=16, drop_rate=0.2,
        ...
        self.dorefine = do_refine

        # dont touch me for now but i matter
        self.cpu = '/cpu:0'
        self.gpu = '/gpu:0'

        # dont touch me for now but i matter less
    ...
    def generate_model_al(self, imgs, gtmaps):
        """ Create the complete graph
            imgs: preproc'ed images. Tensor with B x H x W x C
            gtmaps: gt hmaps. Tensor with B x H/4 x W/4 x C (H, W must be divisible by 4) OR
                              Tensor with B x H x W x C if self.dorefine is True
        """
        ...
        with tf.device(None):  #self.gpu):
            ...
            optimtime = time.time()
            logging.info('---Optim : Done (' + str(int(abs(optimtime - lrtime))) + ' sec.)')
            ...
            minimtime = time.time()
            logging.info('---Minimizer : Done (' + str(int(abs(optimtime - minimtime))) + ' sec.)')
        ...
        endtime = time.time()
        logging.info('Model created (' + str(int(abs(endtime - starttime))) + ' sec.)')
        ...
        with tf.name_scope('Train'):
            ...
            for epoch in range(nEpochs):
                ...
                for i in range(validIter):
                    img_valid, gt_valid, w_valid = next(self.generator)  # XXXAL looks like a bug want self.valid_gen
    ...
    def _graph_hourglass_al_single(self, inputs):  # self const
        """Create the Network
        Args:
            inputs : TF Tensor (placeholder) of shape (None, imnr, imnc, imnchan) size
        """

        (_, imnr, imnc, imnchan) = inputs.shape.as_list()
        assert imnr % 4 == 0, "Image size must be divisible by 4"
        assert imnc % 4 == 0, "Image size must be divisible by 4"
        assert self.nStack == 1, "Single hourglass"
        ...
        with tf.name_scope('model'):
            ...
            if self.dorefine:
                ...
                batch_norm_params = {
                        ...
                        'decay': 0.99,  # elsewhere HG uses 0.9
                        'epsilon': 1e-9,  # elsewhere HG uses 1e-5
                        'scale': True,  # maybe unnec
                        'trainable': True,
                ...
                (_, outnr, outnc, _) = net.shape.as_list()
                assert (outnr, outnc) == (imnr, imnc), "Unexpected size after refine/deconv"
                logging.warning("Adding refinement deconv(s) to network")

            drop[0] = tf.layers.dropout(net, rate=self.dropout_rate, training=self.training, name='dropout')
            ...
            # nb x nStack x gtnr x gtnc x self.outDim OR
            # nb x nStack x imnr x imnc x self.outDim
    ...
    def prediction_layer_dlc(cfg, input, name, num_outputs):
    ...
    def _hourglass(self, inputs, n, numOut, name='hourglass'):
        """ Hourglass Module
        Args:
            inputs	: Input Tensor (nb x gtnr x gtnc x nFeat)
        ...
        with tf.name_scope(name):
            ...
            hwup1 = np.array(up_1.shape.as_list()[1:3])

            # Lower Branch
            low_ = tf.contrib.layers.max_pool2d(inputs, [2, 2], [2, 2], padding='VALID')
            low_1 = self._residual(low_, numOut, name='low_1')
            # low_1: nb x gtnr/2 x gtnc/2 x numOut
            ...
            if not np.array_equal(hwup1, hwlow3 * 2):
                logging.warning("asymmetric up/down legs in hourglass: {:s} vs {:s}".format(np.array2string(hwup1), np.array2string(hwlow3 * 2)))
