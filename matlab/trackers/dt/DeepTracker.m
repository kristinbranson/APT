...
%       'jrcgpuqueue' 'jrcnslots' 'jrcnslotstrack'}; 
  ...
  properties
    ...
    bgTrkMonitor = []  % BgMonitor obj, or []
    bgTrackPoller  % bgTrackWorkerObj for last/current track; or []
    
    % trackres: tracking results DB is in filesys
    adhocTrkFilePathsFromMidx % map from MovieIndex.id to [ ntrk x nview ] cellstrs of trkfile fullpaths
      % adhocTrkFilePathsFromMidx holds the "ad-hoc" tracking results, i.e. those generated by
      % clicking the big "Track" button in the main window of the GUI.  These are
      % not persisted.  A movie can accumulate arbitrarily many ad-hoc
      % trkfiles (one row per tracking bout).  At read time
      % (trackResGetTrkfiles), a movie's persistent trkfile, if it has one,
      % shadows its ad-hoc trkfiles.
    trkPathFromImovAndViewIndex  % [nmovset x nview] cellstr of absolute paths to whole-movie .trk files.
      % Persisted.  Only covers regular (non-GT) movies.
      % Indexed by (iMov, viewIndex).  Empty string means no .trk file stored.
      % A movie has at most one persistent trkfile (per view), unlike the
      % ad-hoc store.  Written only by trackResSetPersistentTrkfile.
...
%         netmode.isHeadTail || netmode==DLNetMode.multiAnimalTDPoseTrx;      
  ...
  methods
    ...
    function result = trackerCreateInfo(obj)
      result = TrackerCreateInfo('DeepTracker', obj.trnNetType, obj.trnNetMode) ;
    end
...
%         end
      ...
      % if isfield(s,'adhocTrkFilePathsFromMidx')
      %   s = rmfield(s,'adhocTrkFilePathsFromMidx');
      % end
...
%         assert(false,'Unsupported tracking modality');
  ...
  methods    
    ...
    % - When tracking is done for a view, adhocTrkFilePathsFromMidx is updated.
    ...
    function trkSpawn_(obj, totrackinfo, backend, varargin)
      ...
      [trackType, do_call_apt_interface_dot_py, projTempDir] = ...
        myparse(varargin, ...
                'trackType', apt.TrackType.track, ...
                'do_call_apt_interface_dot_py',true, ...
                'projTempDir',[]);
      ...
      obj.trkSpawnCore_(totrackinfojobs, ...
                        totrackinfo, ...
                        backend,...
                        'trackStyle',apt.TrackStyle.movie,...
                        'do_call_apt_interface_dot_py',do_call_apt_interface_dot_py,...
                        'projTempDir',projTempDir);
    end
    ...
    function trkSpawnCore_(obj, totrackinfojobs, totrackinfo, backend, varargin)
      ...
      [trackStyle, do_call_apt_interface_dot_py, projTempDir] = ...
        myparse(varargin,...
                'trackStyle',apt.TrackStyle.movie,...
                'do_call_apt_interface_dot_py',true,...
                'projTempDir',[]);
    ...
    function trkSpawnList_(obj,totrackinfo,backend,varargin)
      ...
      obj.trkSpawnCore_(totrackinfo, ...
                        totrackinfo, ...
                        backend, ...
                        'trackStyle',apt.TrackStyle.list);
    end
        ...
        for movi = 1:nMovies,
          ...
          if tffound,
            ...
            if ~gt && obj.isCurrentBoutPersistent ,
              ...
              % recorded in the persisted store.  Ad-hoc bouts write to the
              % session-only ad-hoc store, since their trkfiles typically
              % live in the per-session APT cache dir, which is deleted
              % when APT exits.
              obj.trackResSetPersistentTrkfile(mIdx, trkfiles) ;
            else
              obj.trackResAddAdhocTrkfile(mIdx, trkfiles) ;
            end
...
%     end
  ...
  methods
    function trackResInit(obj)
      % Initialize tracking-results data structures.
      m = containers.Map('keytype','int32','valuetype','any');
      obj.adhocTrkFilePathsFromMidx = m;
      if ~isempty(obj.lObj) && obj.lObj.nmovies > 0 ,
        nmovies = obj.lObj.nmovies ;
        nview = obj.nview ;
        obj.trkPathFromImovAndViewIndex = repmat({''}, nmovies, nview) ;
      else
        obj.trkPathFromImovAndViewIndex = repmat({''}, 0, 0) ;
      end
    end
    
    function trackResAddAdhocTrkfile(obj, mIdx, trkfiles)
      % Add one ad-hoc trkfile (per view) for the given MovieIndex.
      % trkfiles: [1 x nview] cellstr.  Multiple ad-hoc trkfiles can
      % accumulate for a single movie; each call appends one.  Writes only
      % the session-only ad-hoc store (.adhocTrkFilePathsFromMidx), never
      % the persisted one.
      ...
      if m.isKey(id) ,
        ...
        v = cell(0, obj.lObj.nview) ;
      end
      v(end+1,:) = trkfiles(:)' ;
      obj.adhocTrkFilePathsFromMidx(id) = v ;
    end  % function

    function trackResSetAdhocTrkfiles(obj, mIdx, trkfiles)
      % Replace all ad-hoc trkfiles for the given MovieIndex.
      % trkfiles: [ntrk x nview] cellstr.  Writes only the session-only
      % ad-hoc store (.adhocTrkFilePathsFromMidx), never the persisted one.
      assert(isscalar(mIdx)) ;
      assert(iscellstr(trkfiles)) ;  %#ok<ISCLSTR>
      id = mIdx.id32() ;
      obj.adhocTrkFilePathsFromMidx(id) = trkfiles ;
    end  % function

    function trackResSetPersistentTrkfile(obj, mIdx, trkfiles)
      % Set the persistent (whole-movie) trkfile for the given MovieIndex.
      % trkfiles: [1 x nview] cellstr, one trkfile per view.  A movie has
      % at most one persistent trkfile (per view); this overwrites any
      % existing one.  Writes only the persisted store
      % (.trkPathFromImovAndViewIndex), never the ad-hoc one.  GT movies
      % cannot have persistent trkfiles.
      assert(isscalar(mIdx)) ;
      assert(iscellstr(trkfiles)) ;  %#ok<ISCLSTR>
      [iMov, gt] = mIdx.get() ;
      assert(~gt) ;
      viewCount = numel(trkfiles) ;
      for vwi = 1:viewCount ,
        obj.trkPathFromImovAndViewIndex{iMov, vwi} = trkfiles{vwi} ;
      end
    end  % function

    function tf = trackResHasPersistentTrkfile(obj, mIdx)
      % Whether the given MovieIndex has a persistent (whole-movie)
      % trkfile.  Always false for GT movies.
      [iMov, gt] = mIdx.get() ;
      if gt ,
        tf = false ;
        return
      ...
      tf = iMov <= size(paths, 1) && ~all(cellfun(@isempty, paths(iMov, :))) ;
    end  % function

    function [trkfiles,id] = trackResGetTrkfiles(obj,mIdx)
      % Return known .trk file paths for the given MovieIndex.
      % The persistent (whole-movie) trkfile, if there is one, shadows any
      % ad-hoc trkfiles for the same movie.
      id = mIdx.id32() ;
      if obj.trackResHasPersistentTrkfile(mIdx) ,
        iMov = mIdx.get() ;
        trkfiles = obj.trkPathFromImovAndViewIndex(iMov, :) ;
        return
      end
      % Fall back to ad-hoc tracking results
      m = obj.adhocTrkFilePathsFromMidx ;
      if m.isKey(id) ,
        trkfiles = m(id) ;
      else
...
%       end
      ...
      % Collect movie indices that have persisted .trk paths
      paths = obj.trkPathFromImovAndViewIndex ;
      persistedMIdxs = [] ;
      for iMov = 1:size(paths, 1) ,
        if ~all(cellfun(@isempty, paths(iMov, :))) ,
          persistedMIdxs(end+1) = iMov ;  %#ok<AGROW>
        end
      end
  ...
  methods
    function labelerMovieRemoved(obj,edata)
      ...
      if tfLabeledTrainingRowRemoved
        ...
        obj.adhocTrkFilePathsFromMidx = mapKeyRemap(obj.adhocTrkFilePathsFromMidx,mIdxOrig2New);
        obj.trkPathFromImovAndViewIndex = ...
          reindexCellArrayRows_(obj.trkPathFromImovAndViewIndex, mIdxOrig2New) ;
...
newIndices = arrayfun(@(k) mIdxOrig2New(k), keysPos) ;
newIndicesKept = newIndices(newIndices > 0) ;
if isempty(newIndicesKept) ,
  result = repmat({''}, 0, nview) ;
  return
end
nrowNew = max(newIndicesKept) ;
result = repmat({''}, nrowNew, nview) ;
for i = 1:numel(keysPos) ,
  oldIdx = keysPos(i) ;
  newIdx = mIdxOrig2New(oldIdx) ;
  if newIdx > 0 && oldIdx <= size(cellArray, 1) ,
    result(newIdx, :) = cellArray(oldIdx, :) ;
  end
end
end  % function
