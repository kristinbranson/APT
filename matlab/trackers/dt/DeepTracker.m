...
%     end
  ...
  methods
    ...
    function [trkfiles,id] = trackResGetTrkfiles(obj,mIdx)
      ...
      id = mIdx.id32() ;
      [iMov, gt] = mIdx.get() ;
      % Check persisted whole-movie .trk paths first (non-GT only)
      if ~gt ,
    ...
    function trackResSetTrkfiles(obj,mIdx,trkfiles)
      ...
      if ~gt ,
        paths = obj.trkPathFromImovAndViewIndex ;
        if iMov <= size(paths, 1) && ~all(cellfun(@isempty, paths(iMov, :))) ,
          % The source was the persisted array, so update it there.
          for vwi = 1:size(trkfiles, 2) ,
            obj.trkPathFromImovAndViewIndex{iMov, vwi} = trkfiles{1, vwi} ;
          end
          return
        end
      end
