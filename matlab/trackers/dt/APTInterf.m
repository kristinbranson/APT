classdef APTInterf
  ...
  methods (Static)
    ...
    function [codestr,code] = maTrainCodeGenTrnPack(trnID,dllbl,cache,...
      ...
      if maTopDown
        if maTopDownStage1NetMode.isObjDet
          [codestr,code] = APTInterf.bboxTrainCodeGen(trnID,dllbl,cache,...
            errfile,maTopDownStage1NetType,netType,trnjson,maTopDownStage,...
            leftovers{:});
        else          
          assert(false);
        end
...
%     end
    ...
    function [codestr,code] = bboxTrainCodeGen(trnID,dllbl,cache,errfile,...
        netTypeStg1,netTypeStg2,trnjson,stage,varargin)
