...
for n=1:1:size(headData.kine.flyhead.data.coords(i_LantTip,1,:), 3) %for each frame  
    ...
    axisAngleStore_DEGXYZ =[axisAngleStore_DEGXYZ;axisAngleRad./(pi/180)];%axis angle representation of rotation from head aligned with body to current frame - 1st element is angle in degrees, 2:4th elements are xyz rotation axis
