function [vidobj] = MakeTrackingResultsHistogramVideo2(expdirs,trxfile,varargin)
...
[firstframe,endframe,moviefilestr,resvideo,smoothsig,maxv,...
  ...
  CurrMarkerSize,CurrLineWidth,visible,cropframelims,followfly,fps] = ...
  ...
  'cropframelims',[],'followfly',[],'fps',30);
...
for v = 1:nviews,
  ...
  for fidcurr = 1:nfids,
    for flyi = 1:nfliesplot,
      if isnumeric(trxcolor) && isscalar(trxcolor),
        if trxcolor > 0,
          htrx(fidcurr,v,flyi) = plot(hax(v),nan,nan,'.-','Color',lkcolors(fidcurr,:)*trxcolor+(1-trxcolor),'LineWidth',1);
        else
          htrx(fidcurr,v,flyi) = plot(hax(v),nan,nan,'.-','Color',lkcolors(fidcurr,:)*trxcolor,'LineWidth',1);
        end
      else
        htrx(fidcurr,v,flyi) = plot(hax(v),nan,nan,'.-','Color',trxcolor,'LineWidth',1);
      end
    end
  end
  if plotdensity,
  ...
  for fidcurr = 1:nfids,
    ...
    for flyi = 1:nfliesplot,
      hcurr(fidcurr,v,flyi) = plot(hax(v),nan,nan,'.','Color',colorcurr,'LineWidth',CurrLineWidth,'MarkerSize',CurrMarkerSize);
    end
  end
