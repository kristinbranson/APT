function testBsubBackendConfig_(obj, labeler)
  ...
  obj.testText_{end+1} = ''; 
  labeler.notify('updateBackendTestText');
  obj.testText_{end+1} = sprintf('** Testing that host %s can be reached...\n',host); 
  labeler.notify('updateBackendTestText')
  ...
  labeler.notify('updateBackendTestText');
  if status ~= 0,
    obj.testText_{end+1} = 'FAILURE. Error with ping command.'; 
    labeler.notify('updateBackendTestText');
    return
  end
  ...
  if isempty(m),
    obj.testText_{end+1} = 'FAILURE. Could not parse ping output.'; 
    labeler.notify('updateBackendTestText');
    return;
  end
  if str2double(m{1}) == 0,
    obj.testText_{end+1} = sprintf('FAILURE. Could not ping %s:\n',host); 
    labeler.notify('updateBackendTestText');
    return
  end
  ...
  obj.testText_{end+1} = ''; 
  labeler.notify('updateBackendTestText');
  obj.testText_{end+1} = sprintf('** Testing that we can do passwordless ssh to %s...',host); 
  labeler.notify('updateBackendTestText');
  ...
  elseif ~issuccess && isfailure,
    obj.testText_{end+1} = sprintf('FAILURE. Could not create file in CacheDir %s:',cacheDir); 
    labeler.notify('updateBackendTestText');
    return
  else
    obj.testText_{end+1} = 'FAILURE. ssh test failed.'; 
    labeler.notify('updateBackendTestText');
    return
  end

  % test that we can run bjobs
  obj.testText_{end+1} = '** Testing that we can interact with the cluster...'; 
  labeler.notify('updateBackendTestText');
  remotecmd = 'bjobs';
  cmd = wrapCommandSSH(remotecmd,'host',host);
  obj.testText_{end+1} = cmd; labeler.notify('updateBackendTestText');
  [status,result] = apt.syscmd(cmd);
  obj.testText_{end+1} = result; labeler.notify('updateBackendTestText');
  if status ~= 0,
    obj.testText_{end+1} = sprintf('Error running bjobs on %s',host); 
    labeler.notify('updateBackendTestText');
    return
  end
  obj.testText_{end+1} = 'SUCCESS!';
  obj.testText_{end+1} = '';
  obj.testText_{end+1} = 'All tests passed. JRC Backend should work for you.'; 
  labeler.notify('updateBackendTestText');
end  % function
