function testCondaBackendConfig_(obj, labeler)
  ...
  obj.testText_{end+1} = ''; 
  labeler.notify('updateBackendTestText');
  obj.testText_{end+1} = '** Checking for (lack of) Windows...'; 
  labeler.notify('updateBackendTestText');
  if ispc(),
    obj.testText_{end+1} = 'FAILURE. Conda backend is not supported on Windows.'; 
    labeler.notify('updateBackendTestText');
    return
  end
  ...
  obj.testText_{end+1} = ''; 
  labeler.notify('updateBackendTestText');
  obj.testText_{end+1} = '** Checking for conda...'; 
  labeler.notify('updateBackendTestText');
  ...
  [st,~] = apt.syscmd(cmd);
  if st~=0
    obj.testText_{end+1} = sprintf('FAILURE. Error with ''%s''. Make sure you have installed conda and added it to your PATH.',cmd); 
    labeler.notify('updateBackendTestText');
    return;
  end
  obj.testText_{end+1} = 'SUCCESS!'; 
  ...
  obj.testText_{end+1} = ''; 
  labeler.notify('updateBackendTestText');
  obj.testText_{end+1} = sprintf('** Testing conda run -n %s...', obj.condaEnv); 
  labeler.notify('updateBackendTestText');
  ...
  [st,~] = apt.syscmd(cmd);
  if st~=0
    obj.testText_{end+1} = sprintf('FAILURE. Error with ''%s''. Make sure you have created the conda environment %s',cmd, obj.condaEnv); 
    labeler.notify('updateBackendTestText');
    return
  end
  obj.testText_{end+1} = 'SUCCESS!'; 
  ...
  if isempty(gpuid)
    ...
    obj.testText_{end+1} = sprintf('SUCCESS! Found available GPUs.'); 
    labeler.notify('updateBackendTestText');
  end

  obj.testText_{end+1} = '';
  obj.testText_{end+1} = 'All tests passed. Conda Backend should work for you.'; 
  labeler.notify('updateBackendTestText');
end  % function