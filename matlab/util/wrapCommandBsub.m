...
% Convert jobDuration to char for use in ShellCommand
jobDurationAsChar = num2str(jobDuration);

% Build the bsub command using sequential ShellCommand objects
command0 = ...
  apt.ShellCommand({'bsub', '-n', num2str(nslots), '-gpu', 'num=1','-W',jobDurationAsChar, '-q', gpuqueue, '-o', logFilePathRemote, ...
