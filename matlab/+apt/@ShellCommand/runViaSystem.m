function [st,res,warningstr] = runViaSystem(obj, varargin)
...
if doprecommand ,
  precommand = apt.ShellCommand({'export', 'LD_LIBRARY_PATH='}) ;
  cmd1 = apt.ShellCommand.cat(precommand, '&&', obj) ;
else
  cmd1 = obj ;
end
...
% Echo, & run
cmdAsString = cmd.toString() ;
if verbose ,
  fprintf('apt.ShellCommand.runViaSystem(): %s\n',cmd);
end
[st,res] = system(cmd);
