...
% parameters for this run) opens from the menu with populated parameter
% controls, that some commonly-used tracking parameters appear in it,
% that raising the user level in the dropdown never decreases the number
% of parameter controls, and that editing a parameter and pressing Apply
...
% Parameters to spot-check: tracking parameters (AffectsTraining ==
% false, in the ROOT.Track subtree) that should be visible in the dialog
...
editFqn = 'ROOT.Track.NFramesNeighborhood' ;
...
% The dialog should have had some parameter controls at every level
fprintf('%s: shown control counts by level: %s\n', ...
        mfilename(), mat2str(controlCountFromSweepIndex)) ;
assert(controlCountFromSweepIndex(1) >= 3, ...
       'Expected at least 3 parameter controls at the most basic level, found %d', ...
       controlCountFromSweepIndex(1)) ;

% Raising the level should never decrease the number of controls
...
% (e.g. 'ROOT.Track.NFramesNeighborhood').  Control tags are
