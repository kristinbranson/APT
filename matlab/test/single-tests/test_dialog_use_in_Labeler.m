function test_dialog_use_in_Labeler()
  ...
  fprintf('Currently there are %d uses of questdlg() in Labeler.m\n', questdlg_line_count) ;
  if questdlg_line_count > 0
    error('At most 0 calls to questdlg() are allowed in Labeler.m, but it looks like there are %d now', questdlg_line_count) ;
  end
  ...
  fprintf('Currently there are %d uses of warndlg() in Labeler.m\n', warndlg_line_count) ;
  if warndlg_line_count > 0
    error('At most 0 calls to warndlg() are allowed in Labeler.m, but it looks like there are %d now', warndlg_line_count) ;
  end
end  % function
