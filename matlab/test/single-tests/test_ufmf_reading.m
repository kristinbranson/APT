function test_ufmf_reading()
% Test that reading frames from a .ufmf movie is order-independent: a frame read
% out of order must return the same pixels as the same frame read in order.
% UFMF supports random access (keyframes plus per-frame difference boxes), and
% get_readframe_fcn may preload or cache to speed out-of-order reads, so a
% frame's content must not depend on what was read before it.  We read 10
% evenly-spaced frames in ascending order, then read the same 10 in a fixed
% randomly-shuffled order, and require each shuffled read to match.

  % Fix the RNG so the shuffle (and thus the test) is deterministic.
  rng(42) ;

  linuxPath = '/groups/branson/bransonlab/apt/unittest/shortened-alice-movies/movie-1000-frames.ufmf' ;
