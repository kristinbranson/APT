function [err,errL,errR,errB,errreg,yLre,yRre,yBre] = ...
  objfunIntExtRot3(x,yL,yR,yB,crig,lambda,verifyOpts)
...
try
  ...
  fldsRm = setdiff(fieldnames(crigMod.T),fieldnames(crigMod.R));
  crigMod.T = rmfield(crigMod.T,fldsRm);
  crigMod.T.LB = crigMod.T.LB + dTLB;
  crigMod.T.RB = crigMod.T.RB + dTRB;
  crigMod.T.BL = -crigMod.R.LB'*crigMod.T.LB;
  crigMod.T.BR = -crigMod.R.RB'*crigMod.T.RB;
