...
% take 500 frames from each lbl
NFRAMESPERLBL = 580;
tfKeep = false(numel(IsTr),1);
for iLbl = 1:3
  iTrl = find(lblExpFrmTag(:,1)==iLbl);
  iTrl = randsample(iTrl,NFRAMESPERLBL);
  assert(~any(tfKeep(iTrl)));
  tfKeep(iTrl) = true;
end
nnz(tfKeep)
lblExpFrmTagOrig = lblExpFrmTag;
lblExpFrmTag = lblExpFrmTag(tfKeep,:);
IsTr = IsTr(tfKeep);
pGT0 = pGT0(tfKeep,:);
...
for iLbl = 1:3
  iTrl = find(lblExpFrmTag(:,1)==iLbl);
  iTrlTrn = randsample(iTrl,NTRAINPERLBL);
  idxTrn = [idxTrn;iTrlTrn(:)];
end
idxTst = setdiff((1:numel(IsTr))',idxTrn);
...
save(paramsfile2,'tp');
%% Load params
paramsfile2 = 'tp_reg_NOcorrect_20160112.mat';
tp = load(paramsfile2);
tp = tp.tp;
...
DOROTATE = false;
BBOXJITTERFAC = 16;
pGTTrnN = pGTTrnN(randsample(numel(td.iTrn),NTEST),:);
pIni = shapeGt('initTest',[],td.bboxesTst(1:NTEST,:),mdl,[],pGTTrnN,RT,DOROTATE,...
  'bboxJitterFac',BBOXJITTERFAC);
[~,~,~,~,pTstT] = test_rcpr([],td.bboxesTst(1:NTEST,:),td.ITst(1:NTEST,:),...
  tr.regModel,tr.regPrm,tr.prunePrm,pIni);
pTstT = reshape(pTstT,[NTEST RT 14 101]);
...
Shape.vizRepsOverTime(td.ITst,pTstT,iTrl,tr.regModel.model,'fig',gcf,...
  'pGT',td.pGTTst,'regs',tr.regModel.regs,'nr',3,'nc',3);

%% Viz: test labels that have 2 occlusions
iTag2 = find(a.lblExpFrmTag(:,5)==2);
iTag2Tst = intersect(iTag2,td.iTst);
numel(iTag2Tst)
idx = find(ismember(td.iTst,iTag2Tst));
numel(idx)
figure(5);
Shape.vizDiff(td.ITst,td.pGTTst,pTstTRed(:,:,end),tr.regModel.model,...
...
%% GT Labels
GTLABELS = 'f:\cpr\data\jan\151112.mat';
ld = load(GTLABELS);
