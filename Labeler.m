...
%       obj.labeledpostag{end+1,1} = [];
  ...
  methods
    ...
    function movieAdd(obj,moviefile,trxfile)
      ...
      %
      % moviefile: either string or cellstring (when multiView). Can have
      % macros.
      % trxfile: optional. string or cellstring (when multiView) 
      
      %#MVOK      
      
      moviefile = cellstr(moviefile);
      if numel(moviefile)~=obj.nview
        error('Labeler:movieAdd',...
          'Number of moviefiles supplied (%d) must match number of views (%d).',...
          numel(moviefile),obj.nview);
      end
      movfilefull = cellfun(@(x)obj.projLocalizePath(x),moviefile,'uni',0);
      ...
      if numel(trxfile)~=obj.nview
        error('Labeler:movieAdd',...
          'Number of trxfiles supplied (%d) must match number of views (%d).',...
          numel(trxfile),obj.nview);
      end
      if obj.isMultiView
        assert(all(cellfun(@isempty,trxfile)),...
          'Multiview labeling with trx unsupported.');
        nTgt = 1;
      else        
        if isempty(trxfile{1})
          nTgt = 1;
        else
          assert(exist(trxfile{1},'file')>0,...
            'Cannot find trxfile ''%s''.',trxfile);
          tmp = load(trxfile{1});
          nTgt = numel(tmp.trx);
        end
      end
