...
vartypes = {'spherical', 'diag', 'full', 'ppca', 'sparse'};
...
% Make default sparse structure diagonal
if strcmp(covar_type, 'sparse')
  if nargin < 4
    sparsestruct = eye(dim);
  else,
    sparsestruct = ppca_dim;
    sparsestruct = sparsestruct | eye(dim);
  end
  mix.sparsestruct = sparsestruct;
end
...
case 'sparse',
  mix.covars = repmat(sparsestruct, [1 1 mix.ncentres]);
  mix.nwts = mix.ncentres + mix.ncentres*mix.nin + ...
    mix.ncentres*nnz(sparsestruct);
otherwise
